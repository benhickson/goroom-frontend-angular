...
client.on("peer-leave", function(evt) {
  ...
  if(remoteStreams[streamId] != undefined) {
    ...
    // }
    const remoteContainer = document.getElementById(streamId + '_container');
    const nodeId = remoteContainer.closest('.node').dataset.id;
...
function createCameraStream(uid) {
  ...
  localStream.init(function() {
    ...
    // add node to stream to
    const newNodeIdString = addNode(getNextNodeId());
    localStream.play(newNodeIdString); // play the given stream within the local-video div
